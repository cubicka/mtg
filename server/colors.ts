...
function isCostUnitInColors(
  cost: ManaCostUnit,
  colors: MonoColoredMana[]
): boolean {
  ...
  return (
    colors.findIndex(color => color === cost.mana[0]) !== -1 ||
    colors.findIndex(color => color === cost.mana[1]) !== -1
  );
}
...
export function isColorOkay(card: Card, deckColor: DeckColor) {
  ...
  if (
    isPartOfColors(card.cost, deckColor.main.concat(deckColor.splash)) &&
    isWorthyToSplashAble(card, deckColor)
  )
    return true;
